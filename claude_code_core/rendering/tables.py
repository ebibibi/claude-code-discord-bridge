...
Parses GFM pipe-tables and re-renders them as aligned monospace tables with
ASCII borders (``+---+``) for clean display inside a preformatted block.

The borders are ASCII on purpose. Unicode box-drawing (┌─┬┐ │) looks better
in a terminal but is not guaranteed to occupy exactly one column in a chat
client's code-block font — Discord mobile is a known offender — and a border
that is half a column off makes every row below it look crooked. ASCII is
plainer and always aligns. See ``_border_line``.

This lives in core because *every* frontend needs it. Neither Discord nor
Teams renders GFM pipe-tables natively — Teams states outright that
"text-only messages don't support table formatting" — so on both, a markdown
table arrives as a wall of pipes unless it is converted to monospace first.
...
Whether a *client* honours that 2-column assumption is a property of the
client, not of the text, so it is passed in rather than assumed. Discord's
code-block font does not, which is why CJK tables fall back to the vertical
layout there. A frontend that knows its font does is free to say so.
"""
...
DEFAULT_MAX_WIDTH = 55  # Conservative default; frontends pass their own width
...
def render_table(
    ...
    *,
    ...
    Args:
        table: Parsed table, or None (returned as None for caller convenience).
        max_width: Usable display width inside the surface's preformatted block.
        cjk_is_double_width: Whether the surface's monospace font renders CJK
            at exactly twice the width of ASCII. Defaults to False, which
            forces the vertical layout for CJK tables — a table that is
            readable but plain, rather than one whose columns are visibly
            misaligned. Discord's code-block font is one that does not.
    """
    if table is None:
        return None

    if _table_has_cjk(table) and not cjk_is_double_width:
...
def _border_line(position: str, col_widths: list[int]) -> str:
    ...
    Uses ASCII (+, -) rather than Unicode box-drawing: a chat client's
    code-block font is not guaranteed to render ─ and │ at exactly one
    column (Discord mobile is a known case), and a border that is fractionally
    off makes every row below it look crooked. ASCII is plainer and always
    aligns.
    """
