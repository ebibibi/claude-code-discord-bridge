...
| `CCDB_CODEX_MODEL_PROFILES` | Comma-separated `model=profile` mappings. Matching Codex sessions receive `--profile <profile>`, allowing the profile to select a provider, endpoint, and model catalog. Mapped models appear in `/model` autocomplete only while the local profile, optional catalog entry, and declared provider credential are available. | (optional) |
