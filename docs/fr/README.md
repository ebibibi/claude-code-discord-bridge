...
- **Reprise de session** — `/resume` affiche un menu de sélection des sessions récentes (jusqu'à 25) et reprend celle sélectionnée dans un nouveau fil ; fonctionne depuis n'importe quel canal ou fil — crée toujours un nouveau fil dans le canal principal configuré. Le paramètre optionnel `query` permet de rechercher par mot-clé dans le résumé et le répertoire de travail ; `filter: orphaned` n'affiche que les sessions dont le fil Discord a été supprimé (utile pour reprendre un travail passé)
