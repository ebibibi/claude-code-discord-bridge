...
Claude が起動するのは、次のいずれかを満たす場合です:

- メッセージが**メンション不要チャンネル**（またはその配下のスレッド）にある
- そのメッセージで Bot が **@メンション**されている — これは *1 通ごと*に必要です。人間のスレッドに一度 Claude を呼び出しても、そのスレッドが恒久的に Claude スレッドとして登録されることはありません
- そのスレッドを **Bot 自身が作成した** — 新規会話、`/fork`、`/api/spawn` によって作られたセッションスレッド。Discord は作成者を `Thread.owner_id` に記録するため、これらは明確に ccdb 自身のスレッドであり、その中の返信はすべてメンションなしで処理されます
...
##### メンションで Claude が起動したときのスレッドコンテキスト

メンションはたいてい、Claude が見ていない会話の途中に飛んできます。ccdb は、自分が所有していないスレッドで応答する前に、そのスレッドの**直近 7 日間**のトランスクリプトをプロンプトの先頭に付加し、実際に議論されていた内容について答えられるようにします。スレッド全体を読むのは一見自然な発想ですが誤りです — 数か月前から続くスレッドは、大部分が無関係なまま多量のトークンを消費します。そのためウィンドウは 3 つの軸で制限されています: 経過日数、メッセージ件数（200 件）、そして合計サイズ（12,000 文字。**古い方から**切り詰めるため、返信対象となる直近のやり取りは必ず残ります）。

```
CCDB_THREAD_CONTEXT_DAYS=7   # 0 でトランスクリプトを完全に無効化
```

Bot 自身が作成したスレッドは対象外です。そこでのやり取りはすべて見ているため、再送してもトークンを消費するだけです。
...
| `CCDB_THREAD_CONTEXT_DAYS` | メンションで Claude が起動したとき、自分が所有していないスレッドの履歴を何日分プロンプトに付加するか（`0` で無効） | `7` |
