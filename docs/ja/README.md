...
- メッセージ内で Bot が明示的に **@メンション**されている — これは **1 通ごと**に必要です。人間が作ったスレッドに一度 Claude を呼び出しても、そのスレッドが恒久的に Claude スレッドとして登録されることはありません
- **Bot 自身がそのスレッドを作成した** — `_handle_new_conversation`、`/fork`、`/api/spawn` によって作られたセッションスレッド。Discord は作成者を `Thread.owner_id` に記録するため、これらは明確に ccdb 自身のスレッドであり、その中の返信はすべてメンションなしで処理されます。
