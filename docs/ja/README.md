...
- **セッションリジューム** — `/resume` で直近のセッション一覧（最大 25 件）をセレクトメニューで表示し、選択したセッションを新スレッドで再開。任意のチャンネルやスレッドから実行可能 — 常に設定されたメインチャンネルに新スレッドを作成。オプションの `query` パラメータでサマリーや作業ディレクトリをキーワード検索可能。`filter: orphaned` で削除済みスレッドのセッションのみ表示（過去の作業を復活させるのに便利）
